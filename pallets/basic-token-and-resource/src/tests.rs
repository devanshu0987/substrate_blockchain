...
fn register_account_works() {
	ExternalityBuilder::build().execute_with(|| {
		assert_ok!(BasicToken::register_account(Origin::signed(1)));
		assert_eq!(BasicToken::get_balance(1), 21000000);
	})
}

#[test]
fn cant_double_register() {
	ExternalityBuilder::build().execute_with(|| {
		assert_ok!(BasicToken::register_account(Origin::signed(1)));
		assert_noop!(
			BasicToken::register_account(Origin::signed(1)),
			Error::<TestRuntime>::SenderAlreadyRegistered
		);
	})
}
...
fn transfer_works() {
	ExternalityBuilder::build().execute_with(|| {
		...
		assert_ok!(BasicToken::transfer_balance(Origin::signed(1), 2, 100));
...
fn cant_spend_more_than_you_have() {
	ExternalityBuilder::build().execute_with(|| {
		assert_ok!(BasicToken::register_account(Origin::signed(1)));
		assert_noop!(
			BasicToken::transfer_balance(Origin::signed(1), 2, 21000001),
